...
fn c_to_f(v: f64) -> f64   { v * 9.0 / 5.0 + 32.0 }
fn kmh_to_mph(v: f64) -> f64 { v * 0.621371 }
fn mm_to_in(v: f64) -> f64  { v / 25.4 }
fn hpa_to_inhg(v: f64) -> f64 { v * 0.02953 }
...
fn palette(t: f64, theme: Theme) -> Color {
    let h = match theme {
        Theme::Blue => 200.0 + t.clamp(0.0, 1.0) * 80.0,   // cyan(200°) → indigo(280°)
        Theme::Warm => 280.0 + t.clamp(0.0, 1.0) * 120.0,  // indigo(280°) → red(360°) → orange(40°)
    };
    oklch_to_rgb(0.62, 0.14, h)
}

fn cloud_color(_pct: f64) -> Color { Color::DarkGray }
fn temp_color(t: f64, theme: Theme) -> Color     { palette(((t + 15.0) / 45.0).clamp(0.0, 1.0), theme) }
fn wind_color(s: f64, theme: Theme) -> Color     { palette((s / 60.0).clamp(0.0, 1.0), theme) }
fn pressure_color(p: f64, theme: Theme) -> Color { palette(1.0 - ((p - 985.0) / 55.0).clamp(0.0, 1.0), theme) }
...
fn temp_bar(temp: f64, apparent: f64, min: f64, max: f64, width: usize, theme: Theme) -> Vec<Span<'static>> {
    dual_bar(temp, apparent, min, max, width, temp_color(temp, theme))
}

fn wind_bar(speed: f64, gust: f64, min: f64, max: f64, width: usize, theme: Theme) -> Vec<Span<'static>> {
...
fn summary_parts(s: &DaySummary, imperial: bool) -> Vec<(String, String)> {
...
fn print_banner(out: &mut impl IoWrite, main_rgb: (u8, u8, u8), shadow_rgb: (u8, u8, u8)) -> io::Result<()> {
    // 5-row pixel font, 4 cols per letter, 1=filled (██) 0=empty (  )
    // Main pixels use palette low-end color; shadow uses palette high-end color.
...
fn print_overview(out: &mut impl IoWrite, data: &[HourlyData], term_w: usize, imperial: bool, theme: Theme) -> io::Result<()> {
    ...
    // Chart axis label formatters (convert to display units)
...
fn col_title(i: usize, imperial: bool) -> &'static str {
    if imperial {
        match i {
            0 => "TEMP/FEEL °F",
            3 => "RAIN in",
