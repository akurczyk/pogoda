...
pub fn try_place(chars: &mut Vec<char>, col: usize, label: &str, bg: char) {
    ...
    if col < chars.len() && chars[col..end].iter().all(|&c| c == bg) {
        for (j, &ch) in lc[..end - col].iter().enumerate() {
            chars[col + j] = ch;
        }
    }
}
...
pub fn print_one_chart(
    ...
    for (j, ch) in title_inner.chars().enumerate() {
        if left + j < term_w {
            title_line[left + j] = ch;
        }
    }
    if label_w < term_w {
        title_line[label_w] = title_connector;
    }
    write!(
        out,
        "{dim}{}{reset}\n",
    ...
    for r in 0..chart_h {
        ...
        for c in 0..chart_w {
            ...
            let sec_in_row = secondary.and_then(|(sv, s_min, s_max)| {
                ...
                let s_sub =
                    (((s_val - s_min) / s_range).clamp(0.0, 1.0) * total_sub).round() as usize;
                let s_frac = s_sub.saturating_sub(row_bottom).min(8);
                if s_sub > row_bottom && s_sub <= row_top {
                    Some(s_frac)
                } else {
                    None
                }
