...
// Temperature palette range: −15 °C (cold end) … +30 °C (hot end)
const TEMP_COLD_C: f64 = -15.0;
const TEMP_RANGE_C: f64 = 45.0; // 30 − (−15) = 45

// Wind palette range: 0 … 60 km/h
const WIND_MAX_KMH: f64 = 60.0;

// Pressure palette range: 985 … 1040 hPa (low → high)
const PRESSURE_LOW_HPA: f64 = 985.0;
const PRESSURE_RANGE_HPA: f64 = 55.0; // 1040 − 985 = 55

pub fn cloud_color(_pct: f64) -> Color {
    Color::DarkGray
}
pub fn temp_color(t: f64, theme: Theme) -> Color {
    palette(((t - TEMP_COLD_C) / TEMP_RANGE_C).clamp(0.0, 1.0), theme)
}
pub fn wind_color(s: f64, theme: Theme) -> Color {
    palette((s / WIND_MAX_KMH).clamp(0.0, 1.0), theme)
}
pub fn pressure_color(p: f64, theme: Theme) -> Color {
    palette(
        1.0 - ((p - PRESSURE_LOW_HPA) / PRESSURE_RANGE_HPA).clamp(0.0, 1.0),
        theme,
    )
}
...
mod tests {
    ...
    fn oklch_to_rgb_pure_black_approx() {
        ...
        let Color::Rgb(r, g, b) = oklch_to_rgb(0.0, 0.0, 0.0) else {
            panic!("not Rgb")
        };
